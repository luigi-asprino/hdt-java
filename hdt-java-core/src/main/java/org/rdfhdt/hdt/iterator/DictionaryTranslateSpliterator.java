 ...
 * 
 */
public class DictionaryTranslateSpliterator implements Spliterator<TripleString> {
	...
	private BlockingQueue<Block> nextBlocks;
	private long estimatedSize;

	public DictionaryTranslateSpliterator(IteratorTripleID iteratorTripleID, FourSectionDictionary dictionary,
			CharSequence s, CharSequence p, CharSequence o, int blockSize) {

		System.out.println("Spliterator " + blockSize);
	...
	private DictionaryTranslateSpliterator(IteratorTripleID iteratorTripleID,
			DictionaryPFCOptimizedExtractor dictionary, BlockingQueue<Block> nextBlocks, CharSequence s, CharSequence p,
			CharSequence o, int blockSize, long estimatedSize) {
	...
	private void fetchNextBlocks() {
		while (iterator.hasNext()) {
			try {
				nextBlocks.put(Block.fetchBlock(iterator, this.dictionary, blockSize, s, p, o));
	...
	public boolean tryAdvance(Consumer<? super TripleString> action) {
		...
		if (iterator.hasNext()) {
			try {
				current = nextBlocks.poll(10, TimeUnit.MILLISECONDS);
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
		}
	...
	public Spliterator<TripleString> trySplit() {
		if (iterator.hasNext() && estimatedSize > DEFAULT_BLOCK_SIZE * 2) {
			...
			return new DictionaryTranslateSpliterator(iterator, dictionary, nextBlocks, s, p, o, blockSize, splitted);
		}
		return null;
	}
